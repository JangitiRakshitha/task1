...
cars_data <- read.csv("C:\\Users\\vaagdevi\\Downloads\\Cars_data.csv")
...
lm_multi <- lm(Marks ~ Hours_Study + Hours_Sleep, data = students_data)
...
# Predict values
pred_multi <- predict(lm_multi)

# Compare actual vs predicted
cat("\nActual vs Predicted Marks:\n")
print(data.frame(Actual = students_data$Marks, Predicted = round(pred_multi, 2)))